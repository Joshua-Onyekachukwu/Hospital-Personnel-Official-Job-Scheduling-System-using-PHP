...
$sql ="SELECT ID FROM tblpersonel WHERE EmpId=:empid || EmpContactNumber=:empcontno || EmpEmail=:empemail";
...
if($query -> rowCount() > 0){
echo "<script>alert('personel Id or Email or Contact number alreadery registered with another emplyee');</script>";
...
$sql="insert into tblpersonel(DepartmentID,EmpId,EmpName,EmpEmail,EmpContactNumber,Designation,EmpDateofbirth,EmpAddress,EmpDateofjoining,Description,Password,ProfilePic)values(:deptname,:empid,:empname,:empemail,:empcontno,:designation,:empdob,:empadd,:empdoj,:desc,:password,:pic)";
$query=$dbh->prepare($sql);
...
$query->bindParam(':pic',$pic,PDO::PARAM_STR);
 ...
 $query->execute();
   ...
   if ($LastInsertId>0) {
    echo '<script>alert("personel detail has been added.")</script>';
echo "<script>window.location.href ='add-personel.php'</script>";
  }
  else
    {
...
<html lang="en">
   <head>
      <title>HOSPITAL PERSONEL OFFICIAL JOB SCHEDULING SYSTEM || Add Department</title>
...
<option value="<?php echo htmlentities($row2->ID);?>"><?php echo htmlentities($row2->DepartmentName
 ...
 <?php } ?>
                          ...
                          <br>
                           ...
                           <br>
                           <div class="field">
                              <label class="label_field">Personel Address</label>
                           ...
                           <br>
                           <div class="field">
                              <label class="label_field">Personel Date of Joining</label>
